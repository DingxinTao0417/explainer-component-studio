...
const matches=(r,month)=>r.status==='已完成'&&String(r.completedDate||'').startsWith(month+'-');
...
function sourceSheet(h,label){
  ...
  return `<g data-ani-enter>${paper(45,218,225,296,{fold:28,depth:9},h)}${banner(63,183,186,label,t.green,h)}${rows}<rect x="59" y="307" width="198" height="103" rx="4" fill="none" stroke="${t.blue}" stroke-width="3" stroke-dasharray="7 5"/>${text(157,548,'原始记录保留',18,h,'text-anchor="middle" fill="#55749b"')}</g>`;
}
const orderDefaults={
  title:'截取 5 条示意',subtitle:'完成日期在本月，且状态为已完成',footer:'只改变计入范围，原始五条记录仍然保留。',
  month:'2026-09',sourceLabel:'原始表格',headers:['订单号','下单日期','完成日期','状态'],rawHeader:'初次计入',finalHeader:'修正后计入',
  includedLabel:'计入',excludedLabel:'不计入',rawCountLabel:'本局部初次计入',finalCountLabel:'本局部符合',unit:'条',sampleNote:'不是月度总数',rows:initialRows
};
function renderOrder(p,helpers){
  ...
  const rows=p.rows.map((r,i)=>{
    const yy=y+head+i*rowH,ok=matches(r,p.month),statusColor=r.status==='已完成'?t.green:r.status==='已取消'?'#9aa3b1':t.orange;
  ...
  const content=`${sourceSheet(h,p.sourceLabel)}<path d="M258 307L310 238V516L258 410Z" fill="#cceaff" fill-opacity=".65" stroke="#89bffc" stroke-width="2"/>${paper(309,139,920,437,{fold:18,depth:10},h)}${banner(337,65,440,p.title,t.blue,h)}${text(805,101,p.month+' · 原表局部',23,h)}${text(807,130,p.subtitle,fit(p.subtitle,19,388),h)}<rect x="${x}" y="${y}" width="${width}" height="${head+5*rowH}" rx="13" fill="#fff" stroke="${t.ink}" stroke-width="3"/><path d="M${x+13} ${y}H${x+width-13}Q${x+width} ${y} ${x+width} ${y+13}V${y+head}H${x}V${y+13}Q${x} ${y} ${x+13} ${y}Z" fill="#dceeff"/>${rows}${header}${headLast}<g stroke="#7b97bf" stroke-width="1.3" fill="none">${grid}</g><rect x="${x}" y="${y}" width="${width}" height="${head+5*rowH}" rx="13" fill="none" stroke="${t.ink}" stroke-width="2.5"/>${countGroup('raw',p.rawCountLabel,p.rows.length)}${countGroup('final',p.finalCountLabel,count)}<rect x="855" y="595" width="345" height="62" rx="20" fill="#e9f5ff" stroke="#91bdeb" stroke-width="2.5"/>${text(1027,636,p.sampleNote,fit(p.sampleNote,29,310),h,'text-anchor="middle"')}<g data-ani-pop>${text(778,694,p.footer,fit(p.footer,21,910),h,'text-anchor="middle" fill="#55749b"')}</g>`;
  return svgScene(content,h);
}

const compareDefaults={
  title:'图式归纳',subtitle:'从不同例子，找共同思路',footer:'本例办法：补清要求，再检查',leftTitle:'本周进度通知',rightTitle:'截取 5 条示意',
  leftSteps:['补清要求','对照检查'],rightSteps:['补清要求','对照检查'],relationLabel:'比较与提炼',sampleNote:'样本不是月度总数',
  ...
  headers:['订单号','完成日期','状态','计入'],month:'2026-09',rows:initialRows
};
