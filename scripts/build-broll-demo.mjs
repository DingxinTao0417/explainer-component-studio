...
import {components as originals,css as originalCSS} from '../families/broll-graphics.mjs';
import {components as workflows,css as workflowCSS} from '../families/broll-workflows.mjs';
const graphics=[...originals,...workflows],graphicsCSS=originalCSS+'\n'+workflowCSS;
...
const motionEntry=`import {buildBrollMotion as original} from './broll-motion.mjs';import {workflowEffects,buildWorkflowMotion} from './broll-workflow-motion.mjs';export function buildBrollMotion(gsap,root,id,opts){return workflowEffects.some(e=>e.component===id)?buildWorkflowMotion(gsap,root,id+'-motion',opts):original(gsap,root,id,opts);}`;
await build({stdin:{contents:motionEntry+`export {createPreviewController} from './sound-runtime.mjs';`,resolveDir:L},bundle:true,format:'iife',globalName:'BrollRuntime',outfile:resolve(O,'runtime.js'),logLevel:'silent'});
const motionBundle=await build({stdin:{contents:motionEntry,resolveDir:L},bundle:true,format:'iife',globalName:'BrollMotion',write:false,logLevel:'silent'});
const uses={
...
const data=components.map(c=>({id:c.id,name:c.name,category:c.category,use:uses[c.id]||'替换标题、正文与列表；在组件库中编辑内容并预览专属动作。',description:c.description}));
...
await fs.writeFile(resolve(O,'index.html'),index);await fs.writeFile(resolve(O,'scenes.json'),JSON.stringify(data,null,2));console.log(`Built ${components.length} B-roll scenes and local showcase; no video rendered.`);
