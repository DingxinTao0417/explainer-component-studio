...
export const variants={
 ...
 input:[['空白',{state:'empty'}],['输入中',{state:'input'}],['错误提示',{state:'error',value:'',errorText:'请补充具体要求'}]],
 status:[['已完成',{state:'success',label:'已完成'}],['待处理',{state:'pending',label:'待处理'}],['需修改',{state:'error',label:'需修改'}],['已取消',{state:'neutral',label:'已取消'}]],
 ...
 node:[['步骤',{shape:'step'}],['判断',{shape:'decision'}],['起止',{shape:'terminal'}],['已完成',{shape:'step',state:'complete',label:'检查完成',caption:'全部符合要求'}]],
 ...
 document:[['待核对',{rows:[{label:'对象',text:'各组负责人',checked:false},{label:'任务',text:'填写本周完成与待办事项',checked:false},{label:'时间',text:'周五 17:00 前',checked:false},{label:'入口',text:'共享表格',checked:false}]}],['已核对',{rows:[{label:'对象',text:'各组负责人',checked:true},{label:'任务',text:'填写本周完成与待办事项',checked:true},{label:'时间',text:'周五 17:00 前',checked:true},{label:'入口',text:'共享表格',checked:true}]}]],
 cell:[['选中',{selected:true}],['未选中',{selected:false}],['待处理',{text:'待处理',tone:'orange',selected:false}]],
 'table-row':[['选中',{selected:true}],['未选中',{selected:false}],['排除记录',{cells:[{text:'A04'},{text:'—'},{text:'已取消',tone:'gray'},{text:'不计入',tone:'orange'}]}]],
...
export const examples={
 ...
 atom('folder',{x:15,y:210,objectWidth:310,objectHeight:270,name:'项目材料',subtitle:'',fileLabels:['需求','素材','记录']}),
 atom('document',{x:355,y:102,objectWidth:410,objectHeight:490,title:'处理要求',subtitle:'按要求逐项核对'}),
 atom('browser',{x:0,y:0,objectWidth:650,objectHeight:430,heading:'结果预览',tabs:['项目资料'],body:['查看材料与结果。'],items:['需求说明','完成情况']},{x:845,y:75,scale:.59}),
 ...
 atom('window',{x:120,y:60,objectWidth:1040,objectHeight:590,title:'项目要求 · 提交前检查'}),
 atom('address',{x:160,y:140,objectWidth:960,objectHeight:60,address:'workspace.example / requirements'}),
 atom('input',{x:175,y:260,objectWidth:635,objectHeight:80,value:'只统计本月已完成的订单',state:'input'}),
 atom('button',{x:850,y:260,objectWidth:240,objectHeight:80,label:'开始核对',icon:'play'}),
 atom('checkbox',{x:175,y:390,objectWidth:610,objectHeight:65,label:'已明确日期范围和计入条件',state:'checked'}),
 atom('status',{x:880,y:390,objectWidth:210,objectHeight:66,label:'要求完整',state:'success'}),
 atom('progress',{x:175,y:475,objectWidth:800,objectHeight:166,title:'核对进度',value:100,steps:['补充要求','执行','复核'],state:'complete'}),
 ...
 atom('node',{x:70,y:280,objectWidth:250,objectHeight:132,shape:'step',label:'读取订单',caption:'先看完成日期',state:'normal'}),
 atom('node',{x:455,y:235,objectWidth:320,objectHeight:220,shape:'decision',label:'本月已完成？',caption:'日期 + 状态',state:'active'}),
 atom('node',{x:960,y:130,objectWidth:240,objectHeight:140,shape:'terminal',label:'计入',caption:'保留明细',state:'complete'}),
 atom('node',{x:960,y:455,objectWidth:240,objectHeight:140,shape:'terminal',label:'不计入',caption:'保留排除原因',state:'warning'}),
 ...
 atom('callout',{x:100,y:520,objectWidth:600,objectHeight:160,title:'结果可核对',body:'数量与计入明细一起检查。',direction:'none'})
 ]}
};
