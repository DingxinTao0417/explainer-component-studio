...
顶部“背景样式”与组件、动画效果、音效试听并列，可按分类或关键词浏览 8 种舞台背景。点击卡片查看完整背景，可勾选“叠加示例内容”检查文字搭配；点击“用于组件”回到组件目录，随后打开任意组件即可使用所选背景，原有边框保持不变。目录提示条可以查看当前背景或恢复组件原背景。背景选择写入 `background-style` 查询参数，刷新后仍保留；`tab=backgrounds` 可直接打开背景目录。常驻背景动画仍在“动画效果”的“背景”分类中。
